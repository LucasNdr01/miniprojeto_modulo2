...
SELECT
    p.`NumeroPedido`,
    CASE WHEN dtp.sk_tempo IS NULL THEN -1 ELSE dtp.sk_tempo END,
    CASE WHEN dte.sk_tempo IS NULL THEN -1 ELSE dte.sk_tempo END,
    ...
    CASE
        ...
        ELSE CAST(p.`QTD.Itens` AS UNSIGNED)
    END,
    ...
    CASE
        WHEN TRIM(p.`DtHoraIntegracaoERP`) IN ('','-')
          OR TRIM(p.`Dt Separacao Estoque`) IN ('','-') THEN NULL
        ELSE DATEDIFF(DATE(p.`Dt Separacao Estoque`),
            DATE(STR_TO_DATE(p.`DtHoraIntegracaoERP`,'%m/%d/%Y %h:%i %p')))
    END,

    CASE
        WHEN TRIM(p.`Dt Separacao Estoque`) IN ('','-')
          OR TRIM(p.`DtNotaFiscal`) IN ('','-') THEN NULL
        ELSE DATEDIFF(DATE(p.`DtNotaFiscal`),DATE(p.`Dt Separacao Estoque`))
    END,

    CASE
        WHEN TRIM(p.`DtNotaFiscal`) IN ('','-')
          OR TRIM(p.`Dt_Despacho_Transportadora`) IN ('','-') THEN NULL
        ELSE DATEDIFF(DATE(p.`Dt_Despacho_Transportadora`),DATE(p.`DtNotaFiscal`))
    END,

    CASE
        WHEN TRIM(p.`Dt_Despacho_Transportadora`) IN ('','-')
          OR TRIM(p.`DtEntregaCliente`) IN ('','-') THEN NULL
...
LEFT JOIN dim_tempo dtp
    ON dtp.sk_tempo = CAST(DATE_FORMAT(
        STR_TO_DATE(p.`DtHoraPedido`,'%m/%d/%Y %h:%i %p'),
        '%Y%m%d') AS SIGNED)

LEFT JOIN dim_tempo dte
    ON dte.sk_tempo = CASE
...
LEFT JOIN dim_loja dl
    ON dl.chave_loja = CASE
        ...
        WHEN UPPER(TRIM(REPLACE(REPLACE(p.`Loja-Nome`,'/SC',''),'  ',' '))) = 'PATA AMIGA JGUA DO SUL'
